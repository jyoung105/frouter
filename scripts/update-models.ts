...
function fetchJson(
  ...
  return new Promise((resolve, reject) => {
    const headers: Record<string, string> = {
      "User-Agent": "frouter-updater/1.0",
    };
...
async function main() {
  ...
  const missingRankings = allIds.filter(
    (id) => !rankingsById.has(id) && !rankingsById.has(id.replace(":free", "")),
  );
  ...
  if (APPLY) {
    ...
    for (const m of orNew) {
      ...
      if (nimTwin?.aa_intelligence != null)
        entry.aa_intelligence = nimTwin.aa_intelligence;
      if (nimTwin?.aa_speed_tps != null)
        entry.aa_speed_tps = nimTwin.aa_speed_tps;
  ...
  console.log("\n═══ SUMMARY ═══");
  console.log(
    `  NIM:        +${nimNew.length} new, -${nimRemoved.length} removed`,
  );
  console.log(
    `  OpenRouter: +${orNew.length} new, -${orRemoved.length} removed`,
  );
  console.log(
    `  Rankings:   ${missingRankings.length} models missing ranking data`,
  );
  if (
    !APPLY &&
    ...
    console.log(
      "    3. Add SWE-bench scores for new models from artificialanalysis.ai",
    );
  }
}
