...
function getStoredValue<T extends string>(key: string, allowed: readonly T[]) {
...
function getInitialTheme(): Theme {
  const stored = getStoredValue("fr-theme", ["light", "dark"]);
...
function initTheme() {
  ...
  syncFavicons(initial);

  getElement<HTMLButtonElement>("theme-toggle")?.addEventListener("click", () => {
...
const I18N: Record<Locale, TranslationMap> = {
  en: EN_TRANSLATIONS,
  ko: KO_TRANSLATIONS,
};

function isTranslationKey(key: string): key is TranslationKey {
  return Object.prototype.hasOwnProperty.call(EN_TRANSLATIONS, key);
}
...
function applyLocale(locale: Locale) {
  ...
  document.querySelectorAll<HTMLElement>("[data-i18n]").forEach((el) => {
    ...
    if (!key || !isTranslationKey(key)) return;

    el.innerHTML = I18N[locale][key];
  });
...
function initLocale() {
  const stored = getStoredValue("fr-locale", ["en", "ko"]) ?? "en";
  applyLocale(stored);
  getElement<HTMLButtonElement>("lang-toggle")?.addEventListener("click", () => {
    const current = document.documentElement.getAttribute("lang") === "ko" ? "ko" : "en";
...
if (tbody) {
  ...
  getElement("tier-filters")?.addEventListener("click", (e) => {
...
function simulateNimPing() {
  const nimDot = getElement("nim-status");
  const nimPing = getElement("nim-ping");
...
const orStatus = getElement("or-status");
const orPing = getElement("or-ping");
if (orStatus && orPing) {
