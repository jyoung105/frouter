...
test('writeOpenCode merges provider block, sets model, and writes backup', async () => {
  await withTempTargetsModule(async ({ writeOpenCode, home }) => {
    ...
    const outPath = writeOpenCode({ id: 'meta/llama-3.1-8b-instruct' }, 'nvidia');
    ...
    const backups = readdirSync(dirname(configPath)).filter((f) => f.startsWith('opencode.json.backup-'));
...
test('writeOpenCode defaults to env placeholder even when caller passes key', async () => {
  await withTempTargetsModule(async ({ writeOpenCode, home }) => {
    const configPath = join(home, '.config', 'opencode', 'opencode.json');
    writeOpenCode({ id: 'mistralai/mistral-small-3.2-24b-instruct:free' }, 'openrouter', 'sk-or-demo');

    const cfg = JSON.parse(readFileSync(configPath, 'utf8'));
    assert.equal(cfg.provider.openrouter.options.apiKey, '{env:OPENROUTER_API_KEY}');
    assert.equal(cfg.model, 'openrouter/mistralai/mistral-small-3.2-24b-instruct:free');
  });
});
