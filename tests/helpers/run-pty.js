...
function runPythonPty(args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        reject(new Error(`Failed to parse PTY runner JSON: ${err.message}\nRAW:${stdout}\nERR:${stderr}`));
      }
    });
  });
}

export async function runInPty(command, args = [], options = {}) {
  ...
  const result = await runPythonPty([command, ...args], {
    cwd,
    env: runnerEnv,
