...
function prepareFakeBrowserLauncher(homePath) {
  const cmd =
...
function buildInputChunks(sequence, firstDelayMs = 700, stepMs = 120) {
...
async function runWizard({ home, inputChunks, env = {} }) {
...
test('first-run onboarding rejects invalid key prefix and does not persist malformed key', async () => {
  ...
  try {
    ...
    assert.match(result.stdout, /https:\/\/build\.nvidia\.com\/settings\/api-key/);
